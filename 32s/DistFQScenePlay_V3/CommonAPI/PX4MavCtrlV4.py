...
class PX4MavCtrler:
    ...
    def SendGroundSpeed(self,Speed): 
    ...
    def SendHILCtrlMsg(self,ctrls):
        ...
        for i in range(len(ctrls)):
            if i<len(controls):
                controls[i]=ctrls[i]
        if self.isCom:
            self.the_connection.mav.hil_actuator_controls_send(int(time.time()*1000000),controls,1,1)
        else:
    ...
    def SendHILCtrlMsg1(self):
        name = b'hello'
        if self.isCom:
            self.the_connection.mav.debug_vect_send(name, int(time.time()*1000000), 1100, 1500, 1700)
        else:
