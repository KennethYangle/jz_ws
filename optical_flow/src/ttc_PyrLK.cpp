...
vector<vector<double>> CalcTTC::Gaussian()
{
    vector<vector<double>> result(BLOCK_NUM_X, vector<double>(BLOCK_NUM_Y, 0.0));
    std::default_random_engine e;              //引擎
    std::normal_distribution<float> n(BLOCK_NUM_Y/2, 1.5); //均值, 方差，均值是靠近的中心点；方差越小，中心点越集中，方差越大，分布越平均
...
void CalcTTC::safeStrategy(const double ttc_block[BLOCK_NUM_X][BLOCK_NUM_Y], const vector<cv::Point2f> &optical_flow, const int img_width, const int img_height)
{
    ...
    double gaussian_max = 0;
    int gaussin_row = 0;
    int gaussin_col = 0;
    for (int i = 0; i < gaussian_result.size(); i++)
        for (int j = 0; j < gaussian_result[0].size(); j++)
        {
            // gaussian_max = max(gaussian_max,gaussian_result[i][j]);
            if (gaussian_max < gaussian_result[i][j])
            {
                gaussian_max = gaussian_result[i][j];
                gaussin_row = i;
                gaussin_col = j;
            }
        }
    
    cout << "max_gaus_row:" << gaussin_row << endl;
    cout << "max_gaus_col:" << gaussin_col << endl;
    cout << "max_gaus:" << gaussian_result[gaussin_row][gaussin_col] << endl;
    ...
    cout << "ttc_gaus:" << u_TTC[gaussin_row][gaussin_col] << endl;
    cout << "gauss:" << gaussian_result[u_ttc_safe[0]][u_ttc_safe[1]] << endl;
    cout << "ttc_max:" << ttc_max << endl;
    cout << "ttc_u:" << ttc_block_result[u_ttc_safe[0]][u_ttc_safe[1]][3] << endl;
    cout << "u_ttc:" << u_ttc_max << endl;
    cout << "u_ttc_safe_row:" << u_ttc_safe[0] << endl;
    cout << "u_ttc_safe_col:" << u_ttc_safe[1] << endl;
}
...
double CalcTTC::TTC_messgae_V2(const double ttc_block[BLOCK_NUM_X][BLOCK_NUM_Y], const int img_width, const int img_height, const vector<vector<double>> gaussian_result)
{
    ...
    for (int row = 0; row < BLOCK_NUM_X; row++)
    {
        for (int col = 0; col < BLOCK_NUM_Y; col++)
        {
            ...
            if ((ttc_block_result[row][col][3] < ttc_safe) && (ttc_block_result[row][col][3] > 0))
            {
                ...
                if (ttc_max < ttc_block_result[row][col][3])
                {
                    ttc_max = ttc_block_result[row][col][3];
                    // ttc_min_pos[0] = row;
                    // ttc_min_pos[1] = col;
                }
    ...
    cout << "ttc_min:" << ttc_max << endl;
    // cout << "ttc_safe:" << ttc_block_result[u_ttc_safe[0]][u_ttc_safe[1]][3] << endl;
    cout << "ttc_min_pos_row:" << ttc_min_pos[0] << endl;
    cout << "ttc_min_pos_col:" << ttc_min_pos[1] << endl;
    double gaussian_max = 0;
    int gaussin_row = 0;
    int gaussin_col = 0;
    for (int i = 0; i < gaussian_result.size(); i++)
    ...
    cout << "u_ttc_min_pos:" << u_TTC[ttc_min_pos[0]][ttc_min_pos[1]] << endl;
    // cout << "gauss:" << gaussian_result[u_ttc_safe[0]][u_ttc_safe[1]] << endl;
    // cout << "ttc_max:" << ttc_min << endl;
    // cout << "ttc_u:" << ttc_block_result[u_ttc_safe[0]][u_ttc_safe[1]][3] << endl;
    cout << "u_ttc_max:" << u_ttc_max << endl;
    cout << "u_ttc_safe_row:" << u_ttc_safe[0] << endl;
    cout << "u_ttc_safe_col:" << u_ttc_safe[1] << endl;
...
double CalcTTC::OF_messgae(const vector<cv::Point2f> &optical_flow, const int img_width, const int img_height, const vector<vector<double>> gaussian_result)
{
    ...
    for (int row = 0; row < BLOCK_NUM_X; row++)
    {
        for (int col = 0; col < BLOCK_NUM_Y; col++)
        {
            ...
            // flow_max = max(flow_max, flow_average[row][col]);
    ...
    cout << "u_flow_row:" << u_flow_safe[0] << endl;
    cout << "u_flow_col:" << u_flow_safe[1] << endl;
    cout << "u_flow_max:" << u_flow_max << endl;
...
void CalcTTC::imageCb(const sensor_msgs::CompressedImageConstPtr& msg)
{
    ...
    try
    {
        ...
        for( int i=0; i<BLOCK_NUM_X; i++)
        {
            for (int j=0; j<BLOCK_NUM_Y; j++)
            {
                if (kp_cnt[i][j])
                {
                    ...
                    // if(i == 8 && j == 8)
                    // {
                    //     cout << "ttc_block:" << ttc_block[i][j] << endl;
                    // }
                }
            }
        }
        ...
        // int u_ttc_safe[2];//row and col in matrix for safe ttc
        // int u_flow_safe[2];//row and col in matrix for flow

        // safe ttc area!!! cv::rectangle(src, Point(230, 5), Point(510, 295), Scalar(255, 0, 0), 1, LINE_8, 0);
        // cv::rectangle(img_show, cv::Point(u_ttc_safe[0] - 5, u_ttc_safe[1] - 5), cv::Point(u_ttc_safe[0] + 5, u_ttc_safe[1] + 5), cv::Scalar(255, 0, 0), 2);
        // cv::rectangle(img_show, cv::Point(120, 240), cv::Point(130, 250), cv::Scalar(255, 0, 0), 5);
        ...
        // cv::Point start_ttc = cv::Point(img_width / BLOCK_NUM_X * (u_ttc_safe[0] - 0.5), img_height / BLOCK_NUM_Y * (u_ttc_safe[1] - 0.5));
        // cv::Point end_ttc = cv::Point(img_width / BLOCK_NUM_X * (u_ttc_safe[0] + 0.5), img_height / BLOCK_NUM_Y * (u_ttc_safe[1] + 0.5));
        // cv::rectangle(img_show, start_ttc, end_ttc, cv::Scalar(255, 0, 0), 2);

        // cv::Point start_flow = cv::Point(img_width / BLOCK_NUM_X * (u_flow_safe[0] - 0.5), img_height / BLOCK_NUM_Y * (u_flow_safe[1] - 0.5));
        // cv::Point end_flow = cv::Point(img_width / BLOCK_NUM_X * (u_flow_safe[0] + 0.5), img_height / BLOCK_NUM_Y * (u_flow_safe[1] + 0.5));
        // cv::rectangle(img_show, start_flow, end_flow, cv::Scalar(255, 255, 0), 2);
